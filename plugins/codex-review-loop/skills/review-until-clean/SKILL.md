...
2. Start one durable local run:
   ...
   Add `--base <ref>` only when needed. Codex inherits the user's configured model and reasoning effort while the adapter clears notification commands and verifies effective disables for lifecycle hooks, apps, plugins, supported subagent features, and statically discovered MCP servers; add `--isolate-codex-config` only when isolation is intentional. Managed MCP or notification definitions that cannot be overridden fail closed. An active run is resumable with `status`; never start a second run over it.
