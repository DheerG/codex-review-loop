...
2. Start one durable local run:
   ...
   Add `--base <ref>` only when needed. The engine resolves the selected ref to an immutable commit before storing the run, so later repair commits cannot move the comparison boundary. Codex inherits the user's configured model and reasoning effort while the adapter clears notification commands and verifies effective disables for lifecycle hooks, apps, plugins, current and legacy subagent features, and statically discovered MCP servers; add `--isolate-codex-config` only when isolation is intentional. Managed MCP definitions, notification commands, write-capable sandbox/default-permission settings, or forced reviewer features fail closed. An active run is resumable with `status`; never start a second run over it.
