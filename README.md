...
Codex runs in native review mode, explicitly forces its read-only sandbox, and disables lifecycle hooks, apps, plugins, and every supported multi-agent feature variant. Before invoking the reviewer, the adapter verifies that those feature disables are effective, so higher-precedence managed configuration cannot silently restore write-capable tools. It statically reads ordinary user, system, and project TOML to discover active MCP server names, including a selected legacy profile on Codex releases that support it, then disables every name without launching a server, contacting an endpoint, or forwarding URLs, commands, credentials, or transport details. Managed MCP definitions fail closed because CLI overrides cannot supersede them, including machine policy under ProgramData on Windows. The adapter probes the installed Codex feature inventory and omits flags unknown to older compatible releases while inheriting the selected model and reasoning effort. Use `start --isolate-codex-config` only when you deliberately want to ignore user configuration; its temporary feature-probe config stays below the repository's Git runtime-state directory. Codex has no default round cap; other providers stop at 15 rounds unless `--max-rounds` is supplied.
