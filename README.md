...
Codex runs in native review mode, explicitly forces its read-only sandbox, clears legacy notification commands, and disables lifecycle hooks, apps, plugins, and every supported current or legacy multi-agent feature variant. Before invoking the reviewer, the adapter verifies that those feature disables are effective, so higher-precedence managed configuration cannot silently restore write-capable tools. It statically reads ordinary user and system TOML plus project TOML only when the effective project trust is `trusted`, including multiline inline tables, to discover active MCP server names. On Codex releases that support legacy profiles, selection is resolved once across the complete active configuration stack before every layer is inspected. Every server name is disabled without launching a process, contacting an endpoint, or forwarding URLs, commands, credentials, or transport details. Managed MCP definitions, notification commands, write-capable sandbox/default-permission settings, or forced reviewer features fail closed because CLI overrides cannot supersede them, including machine policy under ProgramData or the real Codex home on Windows. Cloud-managed MCP requirements are restrictive identity allowlists and cannot introduce server transports. The adapter probes the installed Codex feature inventory and omits flags unknown to older compatible releases while inheriting the selected model and reasoning effort. Use `start --isolate-codex-config` only when you deliberately want to ignore user configuration; its temporary feature-probe config stays below the repository's Git runtime-state directory while the real managed layers are still inspected. Codex has no default round cap; other providers stop at 15 rounds unless `--max-rounds` is supplied.
