...
Because `codex features list` cannot ignore user config, every run enumerates the binary's static feature names through a private temporary Codex home below Git runtime state. The adapter copies only the existing authentication identity into that home and performs a configuration-only Codex preflight that resolves cloud policy, then exits on a deliberately absent output schema before starting a session, model call, MCP transport, hook, or reviewer tool. Both cloud-managed config fragments and requirements are bounded and checked with the same fail-closed policy, including rejection of cloud-defined MCP transports and forced unsafe features. The actual reviewer reuses that authenticated policy snapshot, and the temporary identity and cache are deleted when the process ends. The installed feature inventory still determines which disable flags are supported by older compatible releases. Codex has no default round cap; other providers stop at 15 rounds unless `--max-rounds` is supplied.
